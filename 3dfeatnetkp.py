...
def compute_descriptors():
    ...
    with tf.Session(config=config) as sess:
        ...
        for iBin in range(0, len(binFiles)):
            ...
            print(num_keypoints,'/',xyz.shape[1])


            with open(os.path.join(args.use_keypoints_from, '{}_kp.bin'.format(fname_no_ext)), 'wb') as f:
                xyz_nms.tofile(f)
...
def nms(xyz, attention):
    ...
    for i in range(num_models):# num_models = 1
        ...
        n_neighbors = min(50, xyz.shape[1])
        nbrs = NearestNeighbors(n_neighbors=n_neighbors, algorithm='ball_tree').fit(xyz[i, :, :])
        ...
        xyz_nms = xyz[i, max_indices, :]
        attention_nms = attention[i, max_indices]
