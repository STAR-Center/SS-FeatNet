...
from data.augment import get_augmentations_from_list, RotateLarge_, Jitter
from data.augment1 import RotateZLarge_
...
def train():
    ...
    rotate_aug = RotateZLarge_(0.6, angle_clip=1.8)
    jit_aug = Jitter()
    ...
    anchor_pl, positive_pl, R_gt_pl = model.get_placeholders(args.data_dim)

    # Ops
    xyz_op, features_op, anchor_attention_op, end_points = model.get_train_model(anchor_pl, positive_pl, is_training, use_bn=USE_BN)
    loss_op, end_points = model.get_loss(xyz_op, features_op, anchor_attention_op, end_points, R_gt_pl)
    train_op = model.get_train_op(loss_op, lr=args.lr, global_step=global_step)
        ...
        for iEpoch in range(args.num_epochs):
            ...
            while True:
                ...
                if anchors is None or anchors.shape[0] != BATCH_SIZE:
                    break
                R_gts = []
                anchors_rot = np.zeros(anchors.shape)
                for i in range(anchors.shape[0]):
                    anchors_rot[i,:,:3], R_gt = rotate_aug.apply(anchors[i,:,:3])
                    R_gts.append(R_gt)
                R_gts = np.stack(R_gts, axis=0)

                #jit
                anchors[:,:,:3] = jit_aug.apply(anchors[:,:,:3])
                anchors_rot[:,:,:3] = jit_aug.apply(anchors_rot[:,:,:3])

                

                xyz, features, train_loss, step, summary, ep, _ = \
                    sess.run([xyz_op, features_op, loss_op, global_step,summary_op, end_points, train_op],
                            feed_dict={anchor_pl: anchors, positive_pl: anchors_rot, R_gt_pl:R_gts,
                                        is_training: True})
                ...
                if False:#step % args.validate_every_n_steps == 0 or step == 1:
