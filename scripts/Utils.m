...
methods(Static)
    ...
    function ax = pcshow_multiple(cloud_all, transforms)        
     ...
     function ax = pcshow_keypoints(model1, xyz1, varargin)
    % Plots keypoint of point clouds
    ...
    %
        ...
        if isempty(inlierIdx)
            ...
            if(proportionCorrect*k-floor(proportionCorrect*k) < 0.0001)
                numCorrectToPlot = floor(proportionCorrect*k);
            else
            ...
            X = X(:, sampled); Y = Y(:, sampled); Z = Z(:, sampled);
            X = X(:); Y = Y(:); Z = Z(:);
            plot3(X, Y, Z, 'b:','LineWidth',.01);            
            
        end
        
        xlabel('x/m','FontSize',40), ylabel('y/m','FontSize',40), zlabel('z/m','FontSize',40)
