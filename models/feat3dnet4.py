...
def pointnet_sa_module(xyz, points, npoint, radius, nsample, mlp, mlp2, mlp3, is_training, scope, bn=True, bn_decay=None,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        #[B,n,k,c]
        # Max pool again
        new_points = tf.reduce_sum(tf.multiply(new_points,attention), axis=[2], keep_dims=True) # [B,n,1,c]
...
def feature_detection_module(xyz, points, num_clusters, radius, is_training, mlp, mlp2, num_samples=64, use_bn=True):
    ...
    # [B,n,k,c]
    attention = conv2d(new_points, 1, [1, 1], stride=[1, 1], padding='VALID', activation=None, bn=False, scope='attention', reuse=False)
